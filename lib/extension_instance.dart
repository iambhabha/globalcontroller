...
import 'package:globalcontroller/router_report.dart';

import 'global_core/src/global_interface.dart';
import 'global_core/src/global_main.dart';
import 'global_core/src/smart_management.dart';
...
extension ResetInstance on GlobalInterface {
...
extension Inst on GlobalInterface {
  T call<T>() => find<T>();
  ...
  void lazyPut<S>(
    ...
    _insert(
      ...
      permanent: permanent,
      builder: builder,
      fenix: fenix ?? Global.smartManagement == SmartManagement.keepFactory,
    );
  }
  ...
  S? _initDependencies<S>({String? name}) {
    ...
    if (!isInit) {
      ...
      if (isSingleton) {
        if (Global.smartManagement != SmartManagement.onlyBuilder) {
  ...
  _InstanceBuilderFactory? _getDependency<S>({String? tag, String? key}) {
    ...
    if (!_singl.containsKey(newKey)) {
      Global.log('Instance "$newKey" is not registered.', isError: true);
      return null;
    } else {
  ...
  S _startController<S>({String? tag}) {
    ...
    if (i is GlobalLifeCycleMixin) {
      i.onStart();
      if (tag == null) {
        Global.log('Initialization of instance "$S"');
      } else {
        Global.log('A new instance "$S" with the tag "$tag" has been created.');
      }
  ...
  void replace<P>(P child, {String? tag}) {
    ...
    place(child, tag: tag, permanent: permanent);
  }
  ...
  bool delete<S>({String? tag, String? key, bool force = false}) {
    ...
    if (!_singl.containsKey(newKey)) {
      Global.log('Instance "$newKey" already removed.', isError: true);
      return false;
    }
    ...
    if (i is GlobalLifeCycleMixin) {
      i.onDelete();
      Global.log('"$newKey" onDelete() called');
    }
    ...
    if (builder.fenix) {
      ...
      if (dep.lateRemove != null) {
        dep.lateRemove = null;
        Global.log('"$newKey" deleted from memory');
        return false;
      } else {
        _singl.remove(newKey);
        if (_singl.containsKey(newKey)) {
          Global.log('Error removing object "$newKey"', isError: true);
        } else {
          Global.log('"$newKey" deleted from memory');
        }
        return true;
  ...
  void reloadAll({bool force = false}) {
    _singl.forEach((key, value) {
      if (value.permanent && !force) {
        Global.log('Instance "$key" is permanent. Skipping reload');
      } else {
        value.dependency = null;
        value.isInit = false;
        Global.log('Instance "$key" was reloaded.');
      }
    });
  }
  ...
  void reload<S>({
    ...
    builder.dependency = null;
    builder.isInit = false;
    Global.log('Instance "$newKey" was restarted.');
  }
...
class _InstanceBuilderFactory<S> {
  ...
  void _showInitLog() {
    if (tag == null) {
      Global.log('The creation of instance "$S"');
    } else {
      Global.log('A new instance "$S" with the tag "$tag" has been created.');
    }
  }
